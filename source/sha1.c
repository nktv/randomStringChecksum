...
void Custom_SHA1_Transform(uint32_t state[5], const unsigned char buffer[64]) {
    uint32_t a, b, c, d, e;
    typedef union {
        unsigned char c[64];
        uint32_t l[16];
...
void Custom_SHA1_Update(Custom_SHA1_CTX* context, const unsigned char* data, size_t len) {
    ...
    if ((j + len) > 63) {
        ...
        Custom_SHA1_Transform(context->state, context->buffer);
        for (; i + 63 < len; i += 64)
            Custom_SHA1_Transform(context->state, &data[i]);
        j = 0;
    }
    else i = 0;
...
void Custom_SHA1_Final(unsigned char digest[20], Custom_SHA1_CTX* context) {
    ...
    c = 0200;
    Custom_SHA1_Update(context, &c, 1);
    while ((context->count[0] & 504) != 448) {
        c = 0000;
        Custom_SHA1_Update(context, &c, 1);
    }

    Custom_SHA1_Update(context, finalcount, 8);
